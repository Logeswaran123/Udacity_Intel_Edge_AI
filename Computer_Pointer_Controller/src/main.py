...
def build_argparser():
    ...
    parser.add_argument("-l",  "--cpu_extension",          required=False, type=str, help="targeted custom layers.", default=None)
    ...
    parser.add_argument("-flag", "--visualization_flag", required=False, nargs='+',
                        ...
                        help="Example: --flag fd fl hp ge (Seperate each flag by space)"
                             "to visualize different model outputs of each frame,"
...
def main():
    ...
    for flag, frame in input_feeder.next_batch():
        ...
        # Gaze detection
        mouse_coordinate, gaze_vector = gaze_estimation_model_object.predict(left_eye_image, right_eye_image, hp_output)
        ...
        if len(visualize_flag) != 0:
            img_final = np.hstack((cv2.resize(frame, (500, 500)), cv2.resize(frame_copy, (500, 500))))
        else:
            img_final = cv2.resize(frame, (500, 500))
