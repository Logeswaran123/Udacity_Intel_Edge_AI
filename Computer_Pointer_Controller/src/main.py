# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 15:40:56 2020
...
def build_argparser():
    ...
    parser.add_argument("-i", "--input", required=True, type=str,
                        help="Path to image or video or CAM")
...
def main():
    ...
    face_detection_model_object = FaceDetectionClass(model_name=model_paths['Face_detection_model'],
    ...
    logger_object.error("Face detection model loaded: time: {:.3f} ms".format(Face_detection_time))
    first_mark = time.time()
    facial_landmarks_detection_model_object.load_model()
    Landmarks_time = (time.time() - first_mark) * 1000
    logger_object.error(
        "Facial landmarks detection model loaded: time: {:.3f} ms".format(Landmarks_time))
    second_mark = time.time()
    head_pose_estimation_model_object.load_model()
    Headpose_time = (time.time() - second_mark) * 1000
    logger_object.error("Head pose estimation model loaded: time: {:.3f} ms".format(Headpose_time))
    third_mark = time.time()
    gaze_estimation_model_object.load_model()
    Gaze_time = (time.time() - third_mark) * 1000
    logger_object.error("Gaze estimation model loaded: time: {:.3f} ms".format(Gaze_time))
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stats.txt'), 'w') as f:
        ...
        f.write("Facial Landmarks model load time: {:.3f} ms".format(Landmarks_time) + '\n')
        f.write("Head Pose Estimation model load time: {:.3f} ms".format(Headpose_time) + '\n')
        f.write("Gaze Estimation model load time: {:.3f} ms".format(Gaze_time) + '\n')
        f.write("Total: {:.3f} ms".format(load_total_time * 1000) + '\n')
