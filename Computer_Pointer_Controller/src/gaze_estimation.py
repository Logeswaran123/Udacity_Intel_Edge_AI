...
class GazeEstimationClass:
    ...
    def predict(self, left_eye, right_eye, head_pose_out):
        ...
        left_eye_image, right_eye_image = self.preprocess_input(left_eye, right_eye)
        results = self.net.infer(inputs={'left_eye_image':left_eye_image, 'right_eye_image': right_eye_image,
                                         'head_pose_angles': head_pose_out})
        
        mouse_coordinate = self.preprocess_output(results, head_pose_out)
